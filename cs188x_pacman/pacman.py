...
def loadAgent(pacman, nographics):
    ...
    for moduleDir in pythonPathDirs:
        #if not os.path.isdir(curDir+'/'+moduleDir):
        if not os.path.isdir(moduleDir):
            continue
...
def replayGame(layout, actions, display, verbose=True):
    import pacmanAgents
    import ghostAgents
...
def runGames(layout, pacman, ghosts, display, numGames, record, numTraining=0, catchExceptions=False, timeout=30):
    ...
    for i in range(numGames):
        ...
        if record:
            ...
            import pickle
            folder = 'recorded/'
            fname = (folder+'recorded-game-%d' % (i + 1)) + '-'.join([str(t) for t in time.localtime()[1:6]])
