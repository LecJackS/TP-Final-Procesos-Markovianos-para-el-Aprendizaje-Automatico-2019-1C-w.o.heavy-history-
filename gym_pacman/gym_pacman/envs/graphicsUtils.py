...
class GraphicsUtils:
    ...
    def begin_graphics(self, width=640, height=480, color=formatColor(0, 0, 0), title=None, process_id=None):
        ...
        #self._root_window.attributes("-alpha", 0.0)
        self._root_window.protocol('WM_DELETE_WINDOW', self._destroy_window)
        self._root_window.title(title or 'Graphics Window')
        # To use with async agent processes
        x_pos = -600 # render game image outside window
        y_pos = 30
        self._root_window.geometry('%dx%d+%d+%d' % (width, height, x_pos, y_pos))
        ...
        #self._root_window.attributes("-alpha", 0.0)
        self._root_window.attributes("-topmost", False)
        if self._Windows:
